...
Descobri o que era bitcoin naquele dia. O preço para recuperar meu banco de dados era de 0,5 BTC.
...
- Acesso root a um servidor linux Ubuntu 18;
...
docker run --privileged -v /var/run/docker.sock:/var/run/docker.sock -v /usr/bin/docker:/usr/bin/docke -it -p 8080:8080 --name cicd cicd:experimental /bin/bash

sudo docker run -d -p 8000:8000 -e "AUTH0_SUBDOMAIN=dev-hco" --name jupyterhub v5 jupyterhub -f jupyterhub_config.py
