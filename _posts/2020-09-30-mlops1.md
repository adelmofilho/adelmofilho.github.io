...
Com os dados na mão, vamos criar um modelo para estimar a chance de doença cardíaca. Vamos utilizar o pacote *ranger* do R para a modelagem sem qualquer feature engineering. O que queremos é um modelo em produção o mais rápido possível agora.
...
Mas, isso seria bem terrível! Reduziríamos nossa audiência a uma fração muito especifica.
...
Uma vez que temos o código da aplicação e o modelo que sera servido por ela, são inúmeras as formas de disponibilizar essa solução.
...
O produto primordial da AWS são suas máquinas virtuais (EC2) e eles oferecem instância até certo tamanho (memória e cpu) e configuração (sistema operacional) de graça!
...
Debito técnico literalmente quer dizer: custo de retrabalho por ter sido adotada uma solução fácil, leia-se, frágil.

Vamos listar então quais seriam os débitos técnicos ou aprimoramentos se está aplicação fosse a público e não só eu, mas um time estivesse envolvido em seu desenvolvimento.
...
- Não é realizado nenhum **monitoramento** do uso do modelo, de seu comportamento e dos recursos por ele consumido.
